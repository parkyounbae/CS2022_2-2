...
void eval(char *cmdline)
{
    ...
    if(bg) {
      struct job_t * run_job = getjobpid(jobs,pid);
      printf("[%d] (%d) %s", run_job->jid, run_job->pid, run_job->cmdline);
    }
		sigprocmask(SIG_SETMASK, &prev,NULL);
		if(!bg){
			pause();
...
void do_bgfg(char **argv)
{
  ...
  if(argv[1][0] == '%') {
    if((target_job = getjobjid(jobs,atoi(argv[1]+1))) == NULL) {
      printf("%s: No such job\n", argv[1]);
      return;
    }
  } else {
    pid_t pid = atoi(argv[1]);
    if((target_job = getjobpid(jobs,pid)) == NULL) {
