...
```c
// leaf 노드의 record
int64_t key; // key 값은 저장한다.
char value[120]; // value 값을 저장한다
```
...
// internal 노드의 record
int64_t key; // key 값
off_t p_offset; // child의 page offset
```
...
```c
// 페이지 구조체
off_t parent_page_offset; // 페이지의 offset
int is_leaf; // leaf 여부
int num_of_keys; // 가지고 있는 key의 개수
char reserved[104]; // 헤더 reserved 공간
off_t next_offset; // leaf -> 오른쪽 형제노드, internal -> child 중 맨 왼쪽 page offset 
union{
    I_R b_f[248]; // internal record
    record records[31]; // record
```

- Header_Page

```c
off_t fpo; // free page pointer 
off_t rpo; // root page pointer
...
pathname 에 해당하는 db파일을 읽어들인다. 해당파일이 존재하면 읽어들이고 존재하지 않는다면 새로 파일을 만든다. 존재하는 파일을 읽었다면 root page를 불러들인다. 
...
off에 해당하는 page를 초기화 하고 db에 저장한다.
...
wbf에 해당하는 페이지를 초기화 하고 db에 저장한다.
...
검색하고 싶은 key 를 가진 leaf의 offset을 반환한다. 
...
찾고싶은 key 를 인자로 받고 find_leaf 함수를 통해 해당 key가 포함된 leaf의 offset을 반환받는다. 이후 key에 해당하는 value 를 반환한다.
...
새로운 파일을 생성하기 위해 헤더를 만들고 저장한다. 해당 헤더를 헤더로 지정한다.
...
트리에 넣을 key와 value값을 전달받는다. 아직 페이지가 없으면 start_new_file 을 통해 페이지를 생성한다. 넣은 키가 이미 트리에 존재하는지 확인하고 존재하면 -1을 반환한다. find_leaf 를 통해 해당하는 key 가 들어갈 leaf 의 offset을 구한다. 이후 만약에 leaf 에 자리가 있다면 insert_into_leaf 를 통해 값을 넣고 끝. leaf에 자리가 없으면 insert_into_leaf_as를 통해 값을 넣는다. 
...
leaf에 들어갈 자리가 있는 상황이다. for문을 통해 해당 key 가 들어갈 인덱스를 찾는다. 인덱스 뒷부분 요소들을 한칸씩 뒤로 밀어 공간을 확보하고 해당 공간에 record를 넣는다. 이후 page 의 num_of_key의 값을 증가시키고 db 에 기록한다. 
...
leaf 에 자리가 없어 분할을 통해 값을 얻어야 하는 상황이다.  먼저 분할을 위해 새로운 페이지를 생성한다. 인덱스를 찾고 임시 배열에 순서에 맞추어 데이터를 저장해둔다. cut 함수를 통해 나눠질 인덱스를 반환받고 old leaf 에 [0]~[split-1] 까지 넣고, new leaf 에는 나머지를 넣는다. 이후 원래 있던 노드, 새로생긴 노드들간의 NEST_OFFSET을 변경해준다. 분할 이후 부모 노드에 새로 생긴 노드의 키값이 올라간다. 이를 insert_into_parent를 통해 넣는다.
...
off_t insert_into_leaf_as(off_t leaf, record inst) {
    ...
    // 새로 생긴것의 다음 노드는 old_leaf 의 다음 노드 로 변경
		// 올드의 다음 노드는 새로운 노드로 설정
		// 올드->노드   올드->새거->노드
...
부모 노드에 요소를 넣는다. 먼저 부모 노드가 0일땐 새로운 루트를 생성해야한다는 뜻이므로 insert_into_new_root를 호출한다. 들어갈 index 를 찾고 자리가 있으면 insert_into_internal로 넣고 없으면 insert_into_internal_as로 스플릿하며 넣는다. 
...
left의 부모 page 에서 left 가 들어갈 left_index 를 찾는다.
...
새로운 루트노드를 생성하고 old_leaf, new_leaf 의 parent 를 각각 새로 생성한 new_root 로 설정하고 저장한다. 이후 루트 노드를 가리키는 nr 을 새로생성한 루트노드로 변경한다.
...
해당 노드에 값이 들어갈 자리가 있기 때문에 인덱스를 찾고 한칸씩 미룬뒤 그 자리에 값을 넣는다. 
...
해당 노드에 들어갈 자리가 없기 때문에 split을 해줘야 한다. 일단 임시 배열에 인덱스에 해당하는 부분을 제외하고 부모 노드의 정보를 저장한다. 이후 건너뛴 자리에 insert 할 key 를 넣는다. cut 함수를 통해 절반의 인덱스를 구하고 old_parent 에는 [0]~[split-1], 새로만든 new_parent 에는 나머지를 넣는다. 이후 나눠준 요소들에 대하여 parent 작업들을 해준다.
...
off_t insert_into_internal_as(off_t bumo, int left_index, int64_t key, off_t newp) {
    ...
    for (i = 0; i < new_parent->num_of_keys; i++) {
		// 새로 만든 new_parent 의 자식들의 parent 를 설정해준다 .
...
삭제시 호출, 비어있거나해당 요소가 없으면 끝. 있으면 find_leaf 를 통해 해당키가 있는 leaf를 찾고 delete_entry 를 통해 삭제.
...
일단 remove_entry_from_page 를 통해 그 키를 제거 한다. 이후 남아있는 노드의 수에 따라 절반 이상일땐 그냥 넘어가고 절반 이하일땐 merge 또는 키를 빌려온다. 
...
이웃의 키를 빌려온다. 일단 해당하는 노드가 맨 왼쪽일 때와 맨 왼쪽이 아닐때로 나뉜다. 맨 왼쪽이 아니고 leaf 가 아니라면 왼쪽 노드의 제일 오른쪽 키를 빌려온다. 맨 왼쪽이 아닌데 leaf 면 왼쪽 leaf 의 마지막 키를 빌려온다. 맨 왼쪽이라면 예외로 오른쪽 노드의 키를 빌려온다. 
...
void redistribute_pages(off_t need_more, int nbor_index, off_t nbor_off, off_t par_off, int64_t k_prime, int k_prime_index) {
    ...
    if (nbor_index != -2) {
        ...
        if (!need->is_leaf) {
            ...
            // 새로 들어온 키가 가리키는 값 = need node 의 제일 왼쪽 노드
            need->b_f[0].p_offset = need->next_offset;
						// need 노드의 제일 왼쪽 = 이웃 노드의 제일 오른쪽 (이웃노드의 제일 오른쪽을 빌려온거니까)
            need->next_offset = nbor->b_f[nbor->num_of_keys - 1].p_offset;
						// need 의 첫번째의 노드를 불러 need의 부모 설정
...
merge 를 해야하는 상황이다. 맨 왼쪽일 때는 오른쪽의 값을 가져오고 나머지는 왼쪽을 가져온다. leaf 가 아니라면 이웃에 합병당할 노드의 자식들을 넘겨주고 부모 포인터를 수정해준다. leaf 일땐 값 가져오고 끝
...
void coalesce_pages(off_t will_be_coal, int nbor_index, off_t nbor_off, off_t par_off, int64_t k_prime) {
    ...
    if (!wbc->is_leaf) {
        ...
        nbor->b_f[point].key = k_prime;
				// 새로 들어간 값의 오프셋 = 지워진 노드의 첫번째 자식 노드
...
## 현재 B+Tree의 Overhead 를 줄이기 위한 디자인

### Overhead 가 일어나는 부분

만약 트리에 계속 제일 큰 요소만 지속적으로 들어간다면 반대쪽에 자리가 있음에도 불구하고 계속 새로운 페이지를 받아온다. 

→ delete 와 마찬가지로 insert 에도 redistribution 동작을 추가한다. 

insert, delete 시에 존재여부만을 판단하기 위해 db_find 를 사용하는데 현재 find 함수에선 내려가면서 해당하는 키를 만나도 무조건 leaf 까지 내려가게 되어있다.

→ 해당키가 존재하는지 아닌지에 대해서만 궁굼할땐 leaf에 내려가는 도중에 해당하는 키를 찾게 되면 거기서 멈추는 함수를 만든다.

merge 가 일어났을때 merge 직후 페이지가 꽉 차있다. 이렇게 되면 다음 insert 때 split 될 확률이 매우 높다. 

→ merge를 할때 두개가 아닌 여러개의 노드를 같이 고려해 merge 이후에 각각의 노드들이 꽉 차있지 않고 적절히 차있을 수 있기 때문에 merge 이후 split 이 일어날 확률이 줄어든다.